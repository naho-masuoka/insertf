...
<div class="container-fluid">
    ...
    <div class="container">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h2><i class="far fa-file-alt mr-2"></i>{{ Auth::user()->department->name2 }}部&nbsp;資料</h2>
            </div>
            <div class="d-none d-lg-block">
                <a href="/files/create/0" class="btn mybtn">ジャンル作成</a>
                <a href="/files" class="btn mybtn">ジャンル一覧</a>
            </div>
        </div>
    </div>
    <!--/nav-->
    <div class="row">
        ...
        <div class="col-md-3 d-none d-lg-block">
            <div class="card mx-auto mb-2" style="width: 20rem;">
                ...
                <div class="card-body">
                    <form method="POST" action="{{route('internal.upload')}}" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <select name="file_id" class="form-group form-control">
                        ...
                        <input type="file" name="files[]" multiple class="form-group form-control-file" required>
                        <button type="submit" class="btn mybtn form-control">アップロード</button>
                    </form>
                </div>
            </div>
                ...
                <ul class="nav nav-tabs" role="tablist">
                    ...
                    <li class="nav-item">
                        ...
                        <a class="nav-link active" id="item{{$key}}-tab" data-toggle="tab" href="#item{{$key}}"
                            ...
                            @else
                            <a class="nav-link" id="item{{$key}}-tab" data-toggle="tab" href="#item{{$key}}" role="tab"
                                ...
                                <a class="nav-link active" id="item{{$key}}-tab" data-toggle="tab" href="#item{{$key}}"
                                    ...
                                    <a class="nav-link" id="item{{$key}}-tab" data-toggle="tab" href="#item{{$key}}"
                                        ...
                                        {{$f->name}}
                                        @else
                                        {{$f->name}}<small>_{{ \App\Department::find($f->department_id)->name2
                                            }}</small>
                                        @endif
                                    </a>
                    </li>
